...
public class PlayerBoard {
	...
	public PlayerBoard(ArrayList<Direction> p, int u, Direction d) {
		...
		for (int i = 0; i < 17; i++) {
			for (int j = 0; j < 9; j++) {
				b[i][j] = new Hexagon(0, startingCoins, 1000, i, j);
			}
		}
	...
	public int getOurPoints(Move p) {
		...
		int newspace = 0, oldspace = 0, points = 0;
	...
	public boolean isHexagonOurs(Hexagon h) {
		for (Hexagon hex : getOurHexagons()) {
	...
	public ArrayList<PossibleMove> getPossibleMoves() {
		...
		for (int i = 0; i < b.length; i++) {
			for (int j = 0; j < b[0].length; j++) {
				if (new PossibleMove(i, j, myCorner, this).isValid()) {
					posMoves.add(new PossibleMove(i,
							j, myCorner, this));
				}
				
				if (new PossibleMove(i, j, myCorner.getLeft(), this).isValid()) {
					log.trace("is valid \n");
					posMoves.add(new PossibleMove(i,
							j, myCorner.getLeft(), this));
				}
				
				if (new PossibleMove(i, j, myCorner.getRight(), this).isValid()) {
					log.trace("is valid \n");
					posMoves.add(new PossibleMove(i,
							j, myCorner.getRight(), this));
				}
				
				if (new PossibleMove(i, j, myCorner.getOpposite(), this).isValid()) {
					log.trace("is valid \n");
					posMoves.add(new PossibleMove(i,
							j, myCorner.getOpposite(), this));
				}
			}
		}
