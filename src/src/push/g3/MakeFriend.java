...
public class MakeFriend extends Player {
	...
	*/
	...
	public Move makeMove(List<MoveResult> previousMoves) {
		...
		if (currentRound == 1) return doGoodTo(possibleBestFriend);
			...
			if (friends.contains(possibleSecondFriend))
				if(!GameEngine.isValidDirectionForCellAndHome(myCorner, possibleSecondFriend)){
					return doGoodTo(myCorner.getRelative(0));
				}else{
					return doGoodTo(possibleSecondFriend);
				}
			else if (friends.contains(possibleThirdFriend))
				if(!GameEngine.isValidDirectionForCellAndHome(myCorner, possibleThirdFriend)){
					return doGoodTo(myCorner.getRelative(0));
				}else{
					return doGoodTo(possibleThirdFriend);
				}
			...
			for(MoveResult pm: previousMoves){
				...
				if (friends.size() > 0) { //keep co-operating with existing friends to sustain relationship
					...
					if(!GameEngine.isValidDirectionForCellAndHome(myCorner, friends.get(0))){
						return doGoodTo(myCorner.getRelative(0));
					}else{
			...
			if(!GameEngine.isValidDirectionForCellAndHome(myCorner, friends.get(1))){
				return doGoodTo(myCorner.getRelative(0));
			}else{
	...
	public Point getMove(Direction friend) { //Set 'from' slots for every move
		...
		while(board[tempFriendX][tempFriendY]==0){
			tempFriendX++;
			if(tempFriendX>16){
				tempFriendX = 0;
			}
		...
		log.error("move will be executed from " + tempFriendX + " , " + tempFriendY+", direction: "+ friend);
