...
public class PlayerBasicAbility {
    ...
    public static HashMap<Direction, Point[]> playerVulnerablePoints;
    ...
    public static void setUpPlayerPoints() {
        ...
        playerVulnerablePoints = new HashMap<Direction, Point[]>();
        ...
        Point[] NWV = { new Point(7, 1), new Point(4, 2)};
        playerVulnerablePoints.put(Direction.NW, NWV);

        Point[] NEV = { new Point(9, 1), new Point(12, 2)};
        playerVulnerablePoints.put(Direction.NE, NEV);

        Point[] WV = { new Point(3, 3), new Point(3, 5)};
        playerVulnerablePoints.put(Direction.W, WV);

        Point[] SWV = { new Point(4, 6), new Point(7, 7)};
        playerVulnerablePoints.put(Direction.SW, SWV);

        Point[] EV = { new Point(13, 3), new Point(13, 5)};
        playerVulnerablePoints.put(Direction.E, EV);

        Point[] SEV = { new Point(12, 6), new Point(9, 7)};
        playerVulnerablePoints.put(Direction.SE, SEV);
    ...
    public static LinkedList<Move> getPossibleGoodMoves(Direction d,int[][] boards){
        ...
        Point[] adjacentPoints = playerAdjacentPoints.get(d);
        int fromX;
        int fromY;
        ...
        for(int i = 0;i<adjacentPoints.length;i++){
            direction = d;
            for(int j = 0;j<playerPoints.size();j++){
                fromX = getFromX(direction,adjacentPoints[i].x);
                fromY = getFromY(direction,adjacentPoints[i].y);
                if(validCell(fromX,fromY)&&boards[fromY][fromX]>0&&getScore(fromX, fromY, adjacentPoints[i].x, adjacentPoints[i].y, d)>0){
                    possibleMoves.add(new Move(fromX, fromY, direction));
                }
                direction = direction.getLeft();
            }
    ...
    public static int getActualScoreChangeForMoveAndPlayerGivenBoard(Move m, Direction player, int[][] board){
        Point from = new Point(m.getX(), m.getY());
        Point to = new Point(m.getNewX(), m.getNewY());
        int numCoins = board[from.y][from.x];
        return getScoreForPosition(player, to)*numCoins - getScoreForPosition(player, from)*numCoins;
    }
    ...
    //doGood does not attempt to bring coins from far away to the friend. We did not find this function to be useful to us,
    //since other players are unlikely to detect such moves, and therefore are unlikely to reciprocate.
    public static LinkedList<Move> doGood(Direction friend, Direction me, int[][] boards){
        ...
        for(int i = 0;i<allPossibleMove.size();i++){
            ...
            if(temp.getDirection().equals(me.getRelative(-1))||
                ...
                if(getScore(temp.getX(), temp.getY(), temp.getNewX(), temp.getNewY(), me) >= 0) //the move does not hurt us!
    ...
    private static final int LARGE_PILE = 3; 
    public static Move getBestMoveForOurPlayerGivenBestMoveFoundSoFar(Direction ourPlayer, Direction theirPlayer, Move bestMoveSoFar, int[][] board) {
        if(theirPlayer.getLeft().equals(ourPlayer) || theirPlayer.getRight().equals(ourPlayer)) {
            Point[] vulnerablePoints = playerVulnerablePoints.get(theirPlayer);
            Direction direction;
            int fromX;
            int fromY;
            LinkedList<Move> allPossibleMoves = new LinkedList<Move>();

            // find the moves which put large piles into vulnerable positions
            for(int i = 0;i<vulnerablePoints.length;i++){
                direction = theirPlayer;
                for(int j = 0;j<playerPoints.size();j++){
                    fromX = getFromX(direction,vulnerablePoints[i].x);
                    fromY = getFromY(direction,vulnerablePoints[i].y);
                    Move possibleMove = new Move(fromX, fromY, direction);
                    if(validCell(fromX,fromY) && board[fromY][fromX] >= LARGE_PILE && getActualScoreChangeForMoveAndPlayerGivenBoard(possibleMove, theirPlayer, board) > 0){
            ...
            LinkedList<Move> movesWeCanMake = new LinkedList<Move>();

            // find the moves we can make
            Move temp;
            for(int i = 0;i<allPossibleMoves.size();i++){
                temp = allPossibleMoves.get(i);
                if(temp.getDirection().equals(ourPlayer.getRelative(-1))||
                        temp.getDirection().equals(ourPlayer.getRelative(0))||
                        temp.getDirection().equals(ourPlayer.getRelative(1))){
                    if(getScore(temp.getX(), temp.getY(), temp.getNewX(), temp.getNewY(), ourPlayer) >= 0) //the move does not hurt us!
                        movesWeCanMake.add(temp);
                }
            }
            ...
            if(movesWeCanMake.size() > 0) {
                for(Move m : movesWeCanMake) {
                    int moveScore = ScoreChart.getScoreGivenMove(theirPlayer, m, board);
    ...
    public static LinkedList<Move> getPossibleBadMoves(Direction enemy, int[][] boards){
        ...
        Point[] adjacentPoints = playerAdjacentPoints.get(enemy);
        ...
        for(int i = 0;i<adjacentPoints.length;i++){
            direction = enemy;
            for(int j = 0;j<playerPoints.size();j++){
                toX = adjacentPoints[i].x+ direction.getDx();
                toY = adjacentPoints[i].y + direction.getDy();
                if(validCell(toX,toY)&&boards[adjacentPoints[i].y][adjacentPoints[i].x]>0&&getScore(adjacentPoints[i].x, adjacentPoints[i].y,toX,toY, enemy)<0){
                    possibleMoves.add(new Move(adjacentPoints[i].x, adjacentPoints[i].y, direction));
                }
                direction = direction.getLeft();
            }
        }  
