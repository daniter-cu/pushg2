...
public class ScoreChart {
    ...
    private final static int LAST_MOVE_WEIGHT = 2;

    private HashMap<Direction, Queue<Integer>> playerScores;

    public static int getScoreGivenMove(Direction d, Move m, int[][] board) {
        ...
        for(int i = 0; i < playerArea.length; i++) {
            if(endPoint.equals(playerArea[i])) {
                return PlayerBasicAbility.getActualScoreChangeForMoveAndPlayerGivenBoard(m, d, board);
            }
        }
