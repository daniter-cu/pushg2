...
public class TitTatPlayer extends AbstractPlayer {
    ...
    private boolean isValidMove(MoveResult m) {
        return isSuccessByBoundsEtc(m) && isSuccessByCount(m) && isValidDirectionForCellAndHome(m.getMove().getDirection(),AbstractPlayer.getHomeofID(m.getPlayerId()));
    }
    ...
    private boolean isSuccessByBoundsEtc(MoveResult m) {
        ...
        if (!m.getMove().getDirection()
                .equals(AbstractPlayer.getHomeofID(m.getPlayerId()).getRelative(0))
                && !m.getMove().getDirection()
                        .equals(AbstractPlayer.getHomeofID(m.getPlayerId()).getRelative(-1))
                && !m.getMove().getDirection()
                        .equals(AbstractPlayer.getHomeofID(m.getPlayerId()).getRelative(1)))
            return false;
        return true;
    ...
    public Move makeMove(List<MoveResult> previousMoves) {
        ...
        ArrayList<Integer> scores = this.updateScores();
        currentRound++;
